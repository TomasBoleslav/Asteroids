...
public:
    ...
    // Throws std::ios_base::failure if files could not be read.
    ...
    // Throws std::ios_base::failure if data could not be read.
...
private:
    ...
    // Read the content of a file. Throws std::ios_base::failure if the file could not be read.
